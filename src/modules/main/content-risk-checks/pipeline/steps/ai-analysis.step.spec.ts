...
describe('AiAnalysisStep', () => {
  ...
  it('succeeds on the second attempt when first response is invalid', async () => {
    promptsRepo.getById.mockResolvedValue(buildPrompt());
    llm.complete
  ...
  it('returns LLM_CALL_FAILED when llm.complete throws (no in-step retry)', async () => {
    promptsRepo.getById.mockResolvedValue(buildPrompt());
  ...
  it('returns ok with attempts=1 on first-shot success', async () => {
    promptsRepo.getById.mockResolvedValue(buildPrompt());
    llm.complete.mockResolvedValueOnce(
